...
exports.handler = async function(event, context) {
  ...
  if (type === 'sms') {
    ...
    const url = `https://api.msg91.com/api/v5/otp?authkey=${MSG91_AUTH_KEY}&mobile=91${mobile}&otp=${otp}&template_id=${MSG91_TEMPLATE_ID}&sender=${MSG91_SENDER_ID}&otp_expiry=10`;
    ...
    try {
      ...
      if (data && data.type === 'success') {
        ...
        return { statusCode: 200, headers, body: JSON.stringify({ success: false, message: data.message || 'SMS failed', debug: data }) };
      }
    } catch(e) {
      console.log('MSG91 SMS error:', e.message);
      return { statusCode: 500, headers, body: JSON.stringify({ success: false, message: 'SMS gateway error', debug: e.message }) };
    }
  }
